...
| Store in remote TSDB (e.g., Ceph) | Store in S3 buckets           |
...
1. pfSense sends logs via syslog to Fluent Bit External(UDP)
2. Fluent Bit parses and forwards to Loki via HTTP
3. Loki stores log chunks in Rook-Ceph buckets
4. Grafana reads from Loki to show pfSense logs on dashboards
5. Mimir collects metrics from other exporters (e.g., node_exporter)
6. Grafana allows users to **correlate logs and metrics** on the same dashboard
